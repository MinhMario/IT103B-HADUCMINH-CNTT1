...
function getFilteredTotalGoals(minGoals, teamPlayers){
    let found=teamPlayers.filter(s =>s.goals>=minGoals);
    let total=0;
    for(let i=0;i<found.length;i++){
         total += found[i].goals;
    }
    console.log(`Tổng số bàn thắng là ${total}`);
};
let min=+prompt("Nhập số bàn thắng tối thiểu");
getFilteredTotalGoals(min,players);